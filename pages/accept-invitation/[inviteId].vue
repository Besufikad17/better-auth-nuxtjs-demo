...
<template>
    <div class="flex items-center justify-center min-h-[calc(100vh-62px)] dark:bg-black bg-white overflow-auto w-full">
        ...
        <div v-if="invitation && toastType !== 'error'"
            class="flex flex-col gap-4 w-1/3 border border-gray-900 dark:border-gray-200 rounded-md p-4">
            <h1 class="text-2xl dark:text-white">Invitation</h1>
            <p class="text-lg dark:text-white"><b>{{ invitation.inviterEmail }}</b> has invited you to join
                <b>{{ invitation.organizationName }}</b> on <b>better-auth</b>.</p>
            <div class="flex items-center justify-center gap-3">
                <button
                    class="bg-white dark:bg-primary-700 dark:text-white px-2 py-1 flex items-center justify-center gap-2 rounded-md"
                    @click="accept">
                    <Icon v-if="acceptInvitationLoading" name="svg-spinners:180-ring-with-bg" />
                    Accept
                </button>
                <button class="bg-red-500 text-white px-2 py-1 flex items-center justify-center gap-2 rounded-md"
                    @click="decline">
        ...
        <div v-else-if="toastType === 'error'"
            class="flex flex-col gap-4 w-1/3 border border-gray-900 dark:border-gray-200 rounded-md p-4">
            <h1 class="text-2xl text-red-500">Invitation Error</h1>
            <p class="dark:text-white">There was an issue with your invitation.</p>
            <p class="dark:text-white">The invitation you're trying to access is either invalid or you don't have the
                correct permissions. Please check your email for a valid invitation or contact the person who sent it.
            </p>
            <NuxtLink to="/user"
                class="bg-transparent border border-gray-900 dark:border-gray-200 flex items-center justify-center px-2 py-1 dark:text-white">
