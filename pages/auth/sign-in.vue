        ...
        <div class="flex flex-col p-8">
            ...
            <div class="flex flex-col gap-4 p-4 border bg-white dark:bg-black border-gray-300 dark:border-primary-700">
                <div class="flex flex-col gap-2">
                    <h1 class="dark:text-white font-semibold">{{ [authType.split('-')[0].charAt(0).toUpperCase() +
                        authType.split('-')[0].slice(1), authType.split('-')[1].charAt(0).toUpperCase() +
                    authType.split('-')[1].slice(1)].join(' ') }}</h1>
                    ...
                    <Textfield v-model="password" name="password" placeholder="Password" type="password"
                        ...
                        <template #label>
                            <div class="flex items-center justify-between w-full mb-2">
                                ...
                                <NuxtLink to="/auth/forgot-password" class="text-sm dark:text-white underline">Forgot
                                    your password?</NuxtLink>
                            </div>
                        </template>
                    </Textfield>
                    <Checkbox v-model="rememberMe" :data="{ label: 'Remember me', value: true }"
                        class="text-sm dark:text-white cursor-pointer accent-black dark:accent-white bg-transparent border dark:border-white" />
                    <button
                        class="flex items-center justify-center w-full gap-2 p-2 text-sm rounded-sm text-white dark:text-black"
                        :class="isLoading ? 'bg-gray-800 dark:bg-gray-300' : 'bg-black dark:bg-white'"
                        :disabled="isLoading" @click="() => login()">
                        Login
                        <Icon v-if="isLoading" name="svg-spinners:90-ring-with-bg" />
                    </button>
                    <button
                        class="flex items-center justify-center w-full gap-2 p-2 text-sm rounded-sm border border-gray-300 hover:border-gray-900 dark:border-primary-700 dark:hover:border-primary-900 dark:text-white"
                ...
                <div v-else class="flex flex-col gap-3">
                    ...
                    <Textfield v-model="confirmPassword" name="confirmPassword" placeholder="Confirm Password"
                        type="password"
                        class="px-2 py-1 border border-gray-300 bg-transparent dark:border-primary-700 focus:outline-gray-900 focus:rounded-sm dark:focus:outline-primary-900 shadow-sm">
                        <template #label>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input name="image" type="file" v-on:change="onFileUpload"
                                class="px-2 py-1 w-full text-gray-300 dark:text-primary-700 border border-gray-300 dark:border-primary-700 shadow-sm rounded-sm" />
                        </div>
                    </div>
                    <button
                        class="flex items-center justify-center w-full gap-2 p-2 text-sm rounded-sm text-white dark:text-black"
                        :class="isLoading ? 'bg-gray-800 dark:bg-gray-300' : 'bg-black dark:bg-white'"
                        :disabled="isLoading" @click="signup">
                        Create an account
                        <Icon v-if="isLoading" name="svg-spinners:90-ring-with-bg" />
                    </button>
                    <button
                ...
                <div class="flex items-center justify-center border-t border-gray-300 dark:border-primary-700 p-4">
                    <span class="text-xs text-gray-500 dark:text-white">Secured by <a href="#" class="text-orange-300"
                            target="_blank">better-auth.</a></span>
                </div>
            </div>
        </div>
    </div>
