<script setup lang="ts">
    ...
    const passkey = ref("");
    const res = client.useListPasskeys();

    if(res.value.error) {
        emit('error', { message: res.value.error.message });
    }
...
<template>
    <div class="bg-white dark:bg-black w-full flex flex-col justify-start gap-4 mt-6">
        <span class="text-xs text-gray-500 dark:text-primary-700">List of passkeys.</span>
        <span v-for="(key, index) in res.data" :key="index" class="text-white">
                {{ key.name }}
        </span>
        <Textfield v-model="passkey" placeholder="My Passkey" name="passkey" type="text"
        ...
        >
            <template #label>
                <label for="passkey" class="text-sm dark:text-white mb-2">New Passkey</label>
            </template>
        </Textfield>
